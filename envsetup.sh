...
- nfxremote: Add git remote for CM Gerrit Review
- nfxgerrit: A Git wrapper that fetches/pushes patch from/to CM Gerrit Review
- nfxrebase: Rebase a Gerrit change and push it again
- aospremote: Add git remote for matching AOSP repository
- cafremote: Add git remote for matching CodeAurora repository.
- mka:      Builds using SCHED_BATCH on all processors
- mkap:     Builds the module(s) using mka and pushes them to the device.
- nfxka:     Cleans and builds using mka.
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the NFX github
        T=$(gettop)
        pushd $T > /dev/null
...
function eat()
{
    ...
    if (adb shell getprop ro.nfx.device | grep -q "$NFX_BUILD");
    then
...
function nfxremote()
{
    git remote rm nfxremote 2> /dev/null
    ...
    NFXUSER=$(git config --get review.review.nfxosp.org.username)
    if [ -z "$NFXUSER" ]
    then
        git remote add nfxremote ssh://review.nfxosp.org:29418/$GERRIT_REMOTE
    else
        git remote add nfxremote ssh://$NFXUSER@review.nfxosp.org:29418/$GERRIT_REMOTE
    fi
    echo You can now push to "nfxremote".
}
...
function makerecipe() {
  ...
  then
    pwd
    nfxremote
    git push nfxremote HEAD:refs/heads/'$1'
  fi
  '
}
...
function nfxrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        ...
        echo "      nfxrebase <path to project> <patch IDs on Gerrit>"
...
function dopush()
{
    ...
    if (adb shell getprop ro.nfx.device | grep -q "$NFX_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
